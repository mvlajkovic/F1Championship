 ...
 * @author Milica
 */
public class ConstructorsStandings extends Application{
     List<Round> champRounds = new ArrayList<>();
    ConstructorChampionship cc;
    ...
    public void start(Stage primaryStage) {
        ...
        btnEnter.setOnAction(e -> {
            ...
            Platform.runLater(new Runnable() {
                @Override
                public void run() {
                    cc = RoundIO.loadConChamp(season);
                    champRounds = RoundIO.loadRounds(season);
                    cc.setChampionshipRounds(champRounds);
                    cc.setStandings(RoundIO.loadCChampionshipPoints(cc.getChampionshipRounds()));
                    champId = RoundIO.loadDChampId(season);
                    cha_champId = RoundIO.loadConChampId(season);
                    standings = RoundUtil.printTeamsPoints(cc.getStandings());
        ...
        declare.setOnAction(e -> {
            ...
            System.out.println(tmpTeam.toString());
            if (tmpTeam.getTeamCode() == 0) {
                cc.declareChampion();
                RoundIO.saveCChamp(cc);
